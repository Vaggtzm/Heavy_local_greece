...
exports.createGithubIssue = functions.https.onRequest(async (req, res) => {
    ...
    const issueTitle = `Sentry Issue: ${payload.event.title}`;
    const issueBody = `Details:\n\n${payload.event.message}\n\nProject: ${payload.project.name}\n\nURL: ${payload.url}`;

    // GitHub API URL
    const repoOwner = 'porfanid';
    const repoName = 'Heavy_local_greece';
    ...
    try {
        ...
        return res.status(200).send({ message: 'GitHub issue created successfully', issue: response.data });
    } catch (error) {
        return res.status(500).send({ message: 'Failed to create GitHub issue', error: error.message });
    }
});