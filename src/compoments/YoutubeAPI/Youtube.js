                ...
                {videos.map(video => (
                    <div className="col-md-4 mb-3" key={video.id.videoId}>
                        <div className="card p-2 w-100">
                            <img className="card-img-top w-100 " src={video.snippet.thumbnails.default.url} alt="Thumbnail" />
                            <div className="card-body">
                                <h5 className="card-title">{video.snippet.title}</h5>
                                <p className="card-text">{video.snippet.description}</p>
                                <a href={`https://www.youtube.com/watch?v=${video.id.videoId}`} className="btn btn-primary" target="_blank">Watch Video</a>
                            </div>
                        </div>
                    </div>
                ))}
